...
with tab1:
    ...
    with c1:
        fig = px.histogram(patients, x="anchor_age", nbins=20, title="Distribuição de Idade")
    ...
    fig = px.box(
        patients, x="gender", y="anchor_age",
        color="gender", title="Distribuição de Idade por Gênero",
    )
    st.plotly_chart(fig, use_container_width=True)
